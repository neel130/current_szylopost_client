...
function Comments({comments}) {
    ...
    const replyComment = (text, commentId) => {
        ...
        setUniqueId(uid)
        fetch(process.env.REACT_APP_BASE_URL + '/replycomment', {
            method: "put",
            headers: {
                "Content-Type": "application/json",
                "Authorization": "Bearer " + localStorage.getItem("jwt")
            ...
            .then(result => {
                  console.warn(result.reply.length)
                  setChngComm(result.reply.length)
  ...
  return (
    ...
    {post?<> 
        ...
        <div style={{marginTop:"20px"}}  className="card home-card">
...
{/*     
                                                ...
                                                <div style={{padding:"0 8px",marginBottom:"100px"}} className="card-content">
                                                        ...
                                                        <div >
                                                            {
                                                                post.comments.map(record => {
                                                                    ...
                                                                    return (
                                                                     ...
                                                                     <div onClick={() => setDeleteCmnt({postid:post._id,commentid: record._id,postedByid:record.postedBy._id,contentpostedBy:post.postedBy._id})} > 
                                                                         <SubComment   record={record} post={post} targetInput={targetInput} chngComm={chngComm} /> </div>    
                                           ...
                                           {post.disablecomment===true? <p></p> :
                                                ...
                                                {post.disablecomment===true? <p style={{color:"#8080808c",fontSize:"13px",marginTop:"10px",marginLeft:"10px"}} >Comments Disabled</p> :
                                                    <form style={{position:"fixed",bottom:"42px",backgroundColor:"white",width:"100%"}}
                                                     onSubmit={(e) => {
                                                        e.preventDefault()
                                                       {commentId?   replyComment(e.target[0].value, commentId)   : makeComment(e.target[0].value, post._id) }
    
                                                    }} >
                                                        <input ref={inputref} value={comment} onChange={(e) => { setComment(e.target.value) }} type="text" placeholder="add a comment" />
                                                    </form>
                                    }
                  ...
                  <CommentSetting  deletCmnt={deletCmnt}  abc={senddata}  />
